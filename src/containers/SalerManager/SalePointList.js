...
import { fetchSalepoints } from "../../store/actions/salepoint";
...
class SalepointList extends Component {
  state = {
    search: "",
    currentPage: 1,
  ...
  componentDidMount = async () => {
    await this.props.fetchSalepoints({ name: null });
  };
  render() {
    ...
    const {
      error,
      loading,
      salepoints: { data, total, skip, take }
    } = this.props;
      ...
      <Layout module="salerManager">
        <MaterialTable
          ...
          onSearchChange={async search => {
            ...
            await this.props.fetchSalepoints({ name: search });
            // }
          }}
          onChangeRowsPerPage={async page => {
            await this.setState({ currentPage: 1 });
            await this.props.fetchSalepoints({ take: parseInt(page), skip: 0 });
          }}
          onChangePage={async page => {
            ...
            await this.props.fetchSalepoints({
              skip: skipper,
              take: parseInt(take)
            });
          }}
...
const mapDistpathToProps = dispatch => {
  return {
    fetchSalepoints: query => dispatch(fetchSalepoints(query))
  };
};

const mapStateToProps = state => {
  return {
    loading: state.salepoints.loading,
    salepoints: state.salepoints.salepoints,
    error: state.salepoints.error || null
  };
};
